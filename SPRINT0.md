...
|----|-----------|----------|------------------------|---------------------|
| US-001 | As a user, I want to create a task so that I can track things I need to do. | High | 2 | A POST `/tasks` request with a `title` creates a new task and returns it with an `id` and `completed=false`. |
...
| US-003 | As a user, I want to update a task so that I can change its title. | High | 2 | A PUT `/tasks/{id}` request with a new `title` updates the task and returns it. Returns 404 if the task doesn't exist. |
